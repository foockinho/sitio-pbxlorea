...
import hashlib
import base64
from txws import parse_hybi07_frames, make_hybi07_frame
...
class WebSocketRequest(Request):
    ...
    def _clientHandshake13(self):
            ...
            clientKey = self.requestHeaders.getRawHeaders("sec-websocket-key", [])
            ...
            fingerprint = clientKey[0] + WEBSOCKET_UUID
            fingerprint = hashlib.sha1(fingerprint)
            serverSignature =  base64.b64encode(fingerprint.digest())

            handshake.append("Sec-WebSocket-Accept: %s" % serverSignature)
    ...
    def renderWebSocket(self):
        ...
        websocket_version = self.requestHeaders.getRawHeaders("sec-websocket-version", [])
        isSecHandshake = self.requestHeaders.getRawHeaders("sec-websocket-key1", [])
        if websocket_version and websocket_version[0] == '13':
            self._clientHandshake13()
        elif isSecHandshake:
...
class WebSocketTransport(object):
    ...
    def write(self, frame):
        ...
        @type frame: C{str}
        """
        self._request.write(make_hybi07_frame(frame))
...
class WebSocketFrameDecoder(object):
    ...
    def dataReceived(self, data):
        ...
        if not data:
            return
        frames, buf_start = parse_hybi07_frames(data)
        for frame in frames:
            self.handler.frameReceived(frame[1])
        self._currentFrameLength = len(buf_start)
        self._data = buf_start
        return
        while True:
